...
bool CHuRecursion::CheckCanHu(char * pCards, int guiIndex[MAX_GUI_NUM] , int guiIndexNum )
{
	...
	if( guiIndexNum > 0 )
	{
		guiNum = GuiNum(pCards, guiIndex );
	}

	//七对返回
	if( CheckPair(pCards,guiNum,needGuiNum) ) return true;

	if(CheckThirteen(pCards,guiNum,needGuiNum)) return true;

	if( !CheckZi(pCards, HasEyes, guiNum, needGuiNum) ) return false;

	if( !CheckColor(&pCards[START_WAN_INDEX], HasEyes, guiNum, needGuiNum) ) return false;

	if( !CheckColor(&pCards[START_TIAO_INDEX], HasEyes, guiNum, needGuiNum) ) return false;

	if( !CheckColor(&pCards[START_TONG_INDEX], HasEyes, guiNum, needGuiNum) ) return false;
